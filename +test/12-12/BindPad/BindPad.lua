...
function BindPadCore.GetBaseForMorphingSpell(spellAction)
   if nil == BindPadCore.morphingSpellCache then
      ...
      for i = 1, BindPadCore.GetSpellNum(bookType), 1 do
         ...
         if "SPELL" == skillType then
           local morphSpellName = GetSpellBookItemName(i, bookType);
           local baseSpellName = GetSpellInfo(spellId);
           if baseSpellName ~= morphSpellName then
   ...
   if BindPadCore.morphingSpellCache[spellAction] then
      return BindPadCore.morphingSpellCache[spellAction];
   else
      return spellAction;
   end
end
