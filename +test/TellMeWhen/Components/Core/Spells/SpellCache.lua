...
local TMW = TMW
TMW.IE.cacheDB = TMW.IE.cacheDB or {}
local cacheDB = TMW.IE.cacheDB
local L = TMW.L
...
function SpellCache:GetExpectedCacheLength()
	return cacheDB.SpellCacheLength
end
...
TMW:RegisterCallback("TMW_OPTIONS_LOADED", function()
	...
	Cache = cacheDB.SpellCache or {}
	cacheDB.SpellCache = Cache
	
	if true then return end

	cacheDB.SpellCacheWoWVersion = cacheDB.SpellCacheWoWVersion or 0
	cacheDB.SpellCacheLength = cacheDB.SpellCacheLength or CONST.MAX_SPELLID_GUESS
	
	if cacheDB.IncompleteSpellCache
	or cacheDB.SpellCacheWoWVersion < clientVersion
	then
		cacheDB.IncompleteSpellCache = true
		...
		TMW:Fire("TMW_SPELLCACHE_EXPECTEDCACHELENGTH_UPDATED", cacheDB.SpellCacheLength)

		if cacheDB.SpellCacheWoWVersion < clientVersion then
			wipe(Cache)
		elseif cacheDB.IncompleteSpellCache then
			for id in pairs(Cache) do
				index = max(index, id)
			end
		end

		cacheDB.SpellCacheWoWVersion = clientVersion
		...
		f:SetScript("OnUpdate", function()
			...
			if not resume(co) then
				cacheDB.IncompleteSpellCache = false
				cacheDB.SpellCacheLength = index
