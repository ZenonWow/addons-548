...
-- Version number
FLIGHTMAP_VERSION   = "5.4.0.0";

-- Size and names for path texture files
FLIGHTMAP_LINE_SIZE = 256;
FLIGHTMAP_TEX_UP    = "Interface\\AddOns\\FlightMap\\art\\FlightMapUp";
FLIGHTMAP_TEX_DOWN  = "Interface\\AddOns\\FlightMap\\art\\FlightMapDown";
...
-- FLIGHTMAP_POI_KNOWN = "Interface\\AddOns\\FlightMap\\art\\UI-Taxi-Icon-Yellow"; For future update (optional)
FLIGHTMAP_POI_OTHER = "Interface\\TaxiFrame\\UI-Taxi-Icon-Gray";

FLIGHTMAP_POI_ALLIANCE = "Interface\\AddOns\\FlightMap\\art\\UI-Taxi-Icon-Deepblue";
FLIGHTMAP_POI_HORDE = "Interface\\AddOns\\FlightMap\\art\\UI-Taxi-Icon-Red";
...
-- LibBabble-Zone-3.0 replaced by MagicBabble
	...
	for k, v in pairs(map) do
		...
		if k ~= newkey and v.Name == name then
			...
			for l, b in pairs(map) do
				-- Fix fields being nil. They shouldn't be nil, so this should be fixed at the source instead of here. Details...
				if not map[l].Costs then 
					map[l].Costs = {}
				end
				if not map[l].Flights then 
					map[l].Flights = {} 
				end
...
local function lSetDefaultData()
	...
	-- Update Flight Database if necessary
	if not FlightMap.build or FlightMap.build < FLIGHTMAP_METADATA.Version then
		FlightMapUtil.autoPrune()
		FlightMapUtil.repairFlightMap()
		
		DEFAULT_CHAT_FRAME:AddMessage(
			"Flight Map and Time Database updated to version " .. FLIGHTMAP_METADATA.Version,
			1.0, 1.0, 1.0);		
		FlightMap.build = FLIGHTMAP_METADATA.Version
	end
	
	
	local c, cf = UnitClassBase("player");
	if cf ~= "DEATHKNIGHT" then
		--DEFAULT_CHAT_FRAME:AddMessage("You are a " .. c .. ", no Acherus for you :-)", 1.0, 1.0, 0.5);		
		FlightMapUtil.StripPoint2("N", "2:622:655"); -- Acherus: The Ebon Hold
	end
end
...
local function lUpdateTooltip(self, zoneName)
    ...
    if not zoneName or zoneName == "" or FlightMap.Opts.noZoneTip then
    ...
    FlightMapTooltip:SetOwner(WorldMapDetailFrame, "ANCHOR_LEFT"); --self
    ...
    if flights > 0 or levels then
        ...
        FlightMapTooltip:SetFrameStrata("DIALOG");
   else
        FlightMapTooltip:Hide();
    end
    ...
    for i = 1, NUM_WORLDMAP_POIS, 1 do
        ...
        if button:IsShown() then
			...
			if GetCurrentMapZone() == 0 then
				...
				--local texture = _G["WorldMapFramePOI" .. i .. "Texture"];
				--Print("POI " .. (button.name or "noname") .. " -> " .. (button.description or "...") .. " #" .. (button.mapLinkID or "?") .. " " .. ((texture and texture:GetTexture()) or "-"))
				button:Hide(); -- Still DEBUG!!				
				--button:SetNormalTexture(FLIGHTMAP_POI_HORDE); --DEBUG!!
				--return true;
			end
                end
            end
...
local function lShowNodePOI(node, data, space, num)
    ...
    if not FlightMapUtil.knownNode(node) then
		...
		button:SetNormalTexture(FLIGHTMAP_POI_HORDE);
	elseif data.Faction == "Alliance" then
		button:SetNormalTexture(FLIGHTMAP_POI_ALLIANCE);
	else	
    ...
    button.data = data;
    button.node = node;
    button:SetPoint("CENTER", "WorldMapDetailFrame", --DetailFrame -- POIFrame
            "BOTTOMLEFT", x, y);
...
local function lUpdateFlightPOIs(zoneName)
	...
	--print("update pois ", continent, " ", mapZone )
...
function FlightMapPOIButton_OnEnter(self)
	--DEFAULT_CHAT_FRAME:AddMessage("Entered a POI", 1.0, 1.0, 0.5);
	
	local x, y = self:GetCenter();
	local parentX, parentY = WorldMapDetailFrame:GetCenter();
	if (x > parentX) then
		FlightPointTooltip:SetOwner(self, "ANCHOR_LEFT");
	else
		FlightPointTooltip:SetOwner(self, "ANCHOR_RIGHT");
	end
	--WorldMapTooltip:SetFrameLevel(self:GetFrameLevel() + 1)	-- help put tooltips in front
	lAddFlightsForNode(FlightPointTooltip, self.node, "");
	FlightPointTooltip:Show();
end

function FlightMapPOIButton_OnLeave(self)
	FlightPointTooltip:Hide();
end
...
function FlightMap_OnSlashCmd(args)
	...
	elseif args == FLIGHTMAP_HARDRESET then
		...
		InterfaceOptionsFrame_OpenToCategory(_G["InterfaceOptionsFlightMapPanel"])
		InterfaceOptionsFrame_OpenToCategory(_G["InterfaceOptionsFlightMapPanel"])
	end
end
...
function FlightMap_OnLoad(self)
	...
	local mytip = CreateFrame("GameTooltip", "FlightPointTooltip", nil, "GameTooltipTemplate")
	mytip:SetFrameStrata("DIALOG")	
	
	mytip = CreateFrame("GameTooltip", "FlightMapTooltip", nil, "GameTooltipTemplate")
	mytip:SetFrameStrata("DIALOG")	
...
function FlightMap_OnEvent(self, event, ...)
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		-- For debug:
		--FlightMap.Babble = MagicBabbleGen()		
	end    
end
