...
FlightMapUtil.getFlightTime = function(source, dest, map)
	-- Dig out the flight time data
	local lmap = false
	if not map then
		map = FlightMapUtil.getFlightMap()
		lmap = true
	end
	
	local value, exact = nil, false
	
	if map[source] and map[source].Flights[dest] then
		value = map[source].Flights[dest]
		exact = true		
	elseif map[dest] and map[dest].Flights[source] then
		value = map[dest].Flights[source]
	end
	
	if (not value) and lmap then
		local emap = FlightMapUtil.getFlightMap(true)
		if emap[source] and emap[source].Flights[dest] then
			value = emap[source].Flights[dest]
		elseif emap[dest] and emap[dest].Flights[source] then
			value = map[dest].Flights[source]
		end
	end

	-- Ride Like the Wind (Level 21) - Increase flight path travel speed by 25%. Replaces Have Group, Will Travel.
	if value and IsInGuild() and ( (GetGuildLevel() or 0) > 20) then
		value = value / 1.25
	end
...
FlightMapUtil.setFlightTime = function(source, dest, value, map)
	-- Dig out the flight time data
	if not map then
		map = FlightMapUtil.getFlightMap();
	end
	...
	-- Ride Like the Wind (Level 21) - Increase flight path travel speed by 25%. Replaces Have Group, Will Travel.
	if IsInGuild() and ( (GetGuildLevel() or 0) > 20) then
		value = value * 1.25
	end	
	map[source].Flights[dest] = value;	
end
...
FlightMapUtil.getFlightMap = function(opposite)
	local faction = UnitFactionGroup("player");    
	if not faction then return {}; end
	if opposite then
		if faction == FLIGHTMAP_ALLIANCE then
			faction = FLIGHTMAP_HORDE
		else
			faction = FLIGHTMAP_ALLIANCE
		end
	end
...
FlightMapUtil.StripPoint2 = function(faction, point)
	...
	if faction == "A" or faction == "N" then
		stripmap(FlightMap[FLIGHTMAP_ALLIANCE] or {})
	end
	if faction == "H" or faction == "N" then
		stripmap(FlightMap[FLIGHTMAP_HORDE] or {})
	end
...
FlightMapUtil.autoPrune = function()
	if not FlightMap.build then FlightMap.build = 0; end
	local build = FlightMap.build
	local lStripPoint2 = FlightMapUtil.StripPoint2
	
	for k,v in pairs(FLIGHTMAP_METADATA.Prune) do
		if v > build then
			lStripPoint2(FLIGHTMAP_METADATA.Faction[k], k)
		end
	end
end
...
FlightMapUtil.repairFlightMap = function()
	...
	if not FlightMap[faction] then FlightMap[faction] = {} end
		...
		elseif (v.Faction == nil or v.Faction == faction) then
			-- repair certain aspects
			FlightMap[faction][k].Zone = v.Zone
			FlightMap[faction][k].Continent = v.Continent
			if FlightMap[faction][k].Location.Continent.y < 0.0001 then
				FlightMap[faction][k].Location.Continent.x = v.Location.Continent.x
				FlightMap[faction][k].Location.Continent.y = v.Location.Continent.y
			end
			if FlightMap[faction][k].Location.Zone.y < 0.0001 then
				FlightMap[faction][k].Location.Zone.x = v.Location.Zone.x
				FlightMap[faction][k].Location.Zone.y = v.Location.Zone.y				
			end			
		end
        end

	-- todo: remove obsolete points from database
end
...
FlightMapUtil.knownNode = function(node, learning)
    ...
    local realm = GetRealmName() --GetCVar("realmName");
