...
function module:VIGNETTE_ADDED(event, instanceid, mysterious_number)
	if not instanceid then
		-- ...just in case
		core:Print("No Vignette instanceid")
		return
	end
	...
	if not name then
		core:Print("Failed to pull Vignette Name do to blizzard bug with instanceid" ..instanceid)
	...
	if iconid == 40 then
		core:Print("Chest Nearby: "..name)
		PlaySoundFile("Sound\\Doodad\\PortcullisActive_Closed.wav", "Master")
	end
end
