...
Mappy.ObjectIconsNormalSmallPath = "Interface\\Minimap\\ObjectIcons"
Mappy.ObjectIconsHighlightSmallPath = "Interface\\Addons\\Mappy\\Textures\\ObjectIcons_On"

Mappy.ObjectIconsNormalLargePath = "Interface\\Addons\\Mappy\\Textures\\ObjectIcons_Large"
Mappy.ObjectIconsHighlightLargePath = "Interface\\Addons\\Mappy\\Textures\\ObjectIcons_On_Large"
...
function Mappy:InitializeMinimap()
	...
	self:ClonePlayerArrow()
	
	self:InitializeAttachedFrames()
	self:FixWatchFramePosition()
...
function Mappy:HookAttachedFrames()
	...
	or not self.CurrentProfile.DetachManagedFrames then return end
...
function Mappy:FixWatchFramePosition()
	hooksecurefunc(WatchFrame, "SetPoint", function (frame, anchor, relativeTo, relativePoint, offsetX, offsetY)
		if (relativeTo == "MinimapCluster" or relativeTo == MinimapCluster)
		and not WatchFrame:IsProtected()
		and offsetY
		and offsetY > 0 then
			WatchFrame:SetPoint(anchor, relativeTo, relativePoint, offsetX, 0)
		end
	end)
...
function Mappy:ClonePlayerArrow()
	self.MappyPlayerArrow = CreateFrame("Model", nil, MinimapCluster)
	
	self.MappyPlayerArrow:SetPoint("CENTER", MinimapCluster, "CENTER")
	self.MappyPlayerArrow:SetWidth(32)
	self.MappyPlayerArrow:SetHeight(32)
	self.MappyPlayerArrow:SetScale(1)
	
	self.MappyPlayerArrow:SetModel("Interface\\Minimap\\MinimapArrow.mdx")
	self.MappyPlayerArrow:SetModelScale(0.7)
	
	self.SchedulerLib:ScheduleRepeatingTask(0.02, self.SyncPlayerArrow, self)
end

function Mappy:SyncPlayerArrow()
	local vScale = MinimapCluster:GetEffectiveScale()
	local vHyp = ((GetScreenWidth() * vScale) ^ 2 + (GetScreenHeight() * vScale) ^ 2) ^ 0.5
	local vArrowWidth = self.MappyPlayerArrow:GetRight() - self.MappyPlayerArrow:GetLeft()
	local vArrowHeight = self.MappyPlayerArrow:GetTop() - self.MappyPlayerArrow:GetBottom()
	local vCoordRight = vArrowWidth / vHyp
	local vCoordTop = vArrowHeight / vHyp
	local vPixelCoordWidth = vCoordRight / vArrowWidth
	local vPixelCoordHeight = vCoordTop / vArrowHeight
	
	self.MappyPlayerArrow:SetPosition(0.5 * vCoordRight + vPixelCoordWidth, 0.5 * vCoordTop + 1.5 * vPixelCoordHeight, 0)
	
	if self.CurrentProfile.RotateMinimap then
		self.MappyPlayerArrow:SetFacing(0)
	else
		self.MappyPlayerArrow:SetFacing(GetPlayerFacing())
	end
end
...
function Mappy:ConfigureMinimap()
	...
	local vPlayerArrowScale = self.CurrentProfile.MinimapSize / 140
	 
	if vPlayerArrowScale > 1.2 then
		vPlayerArrowScale = 1.2
	elseif vPlayerArrowScale < 1 then
		vPlayerArrowScale = 1
	end
	
	if self.MappyPlayerArrow then
		self.MappyPlayerArrow:SetScale(vPlayerArrowScale)
	end
...
function Mappy:StartGatherFlash()
	...
	if not self.ObjectIconsNormalCache then
		...
		self.ObjectIconsNormalCache:SetAllPoints()
		self.ObjectIconsNormalCache:Hide()
		
		self.ObjectIconsHighlightCache = MinimapCluster:CreateTexture(nil, "BACKGROUND")
		self.ObjectIconsHighlightCache:SetAllPoints()
		self.ObjectIconsHighlightCache:Hide()
	end
	...
	self.SchedulerLib:ScheduleUniqueRepeatingTask(0.15, self.UpdateGatherFlash, self)
end
