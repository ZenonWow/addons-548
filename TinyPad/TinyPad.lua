...
function tinypad:PLAYER_LOGIN()
  self.PLAYER_LOGIN = nil
	...
	-- quest window is different for MoP and WoD
	if select(4,GetBuildInfo())==50400 then -- MoP
		...
		local old_QuestMapLogTitleButton_OnClick = QuestMapLogTitleButton_OnClick
	...
	-- for 1.x -> 2.x transition, move bindings to properly-named bindings
	local bindsMoved
	for oldBind,newBind in pairs({["TinyPad Toggle"]="TOGGLE", ["TinyPad Search"]="SEARCH"}) do
		local key = GetBindingKey(oldBind)
		if key then
			SetBinding(key,"TINYPAD_"..newBind)
			bindsMoved = true
		end
	end
	if bindsMoved then
...
function tinypad:OnTextChanged()
	tinypad.undo:SetEnabled(tinypad:GetPageText()~=editBox:GetText())
end
			...
			if TradeSkillFrame and TradeSkillFrame:IsVisible() then -- if tradeskill up, see if it's something to jump to
				...
				for i=1,numRecipes do
					...
					if tradeLink and GetItemInfo(tradeLink)==itemName then
						TradeSkillFrame_SetSelection(i) -- clicked link is in open tradeskill, select it
						local range=TradeSkillListScrollFrame:GetVerticalScrollRange() -- and scroll to it
...
function tinypad:UpdateButtonStates()
	...
	tinypad.undo:SetEnabled(editBox:GetText()~=pages[current])
end
