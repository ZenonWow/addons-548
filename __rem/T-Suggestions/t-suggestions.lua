...
Tsuggest= {
	...
	["Leveling"] = {
		...
		{ 20, "Up to 20: Loch Modan\nWestfall\nDarkshore\nBloodmyst Isle\nSilverpine Forest\nThe Barrens\nGhostlands" },
		{ 25, "Up to 25: Wetlands\nRedridge Mountains\nAshenvale\nThe Barrens\nStonetalon Mountains\nHillsbrad Foothills" },
		{ 28, "Up to 28: Duskwood\nWetlands\nAshenvale\nStonetalon Mountains\nThousand Needles" },
		{ 31, "Up to 31: Duskwood\nThousand Needles\nAshenvale" },
		{ 35, "Up to 35: Thousand Needles\nStranglethorn Vale\nAlterac Mountains\nArathi Highlands\nDesolace" },
		{ 40, "Up to 40: Stranglethorn Vale\nDesolace\nBadlands\nDustwallow Marsh\nSwamp of Sorrows" },
		{ 43, "Up to 43: Tanaris\nStranglethorn Vale\nBadlands\nDustwallow Marsh\nSwamp of Sorrows" },
		{ 45, "Up to 45: Tanaris\nFeralas\nThe Hinterlands" },
		{ 48, "Up to 48: Tanaris\nFeralas\nThe Hinterlands\nSearing Gorge" },
		{ 51, "Up to 51: Tanaris\nAzshara\nBlasted Lands\nSearing Gorge\nUn'Goro Crater\nFelwood" },
		{ 55, "Up to 55: Un'Goro Crater\nFelwood\nBurning Steppes\nBlasted Lands\nWestern Plaguelands" },
		{ 58, "Up to 58: Winterspring\nBurning Steppes\nWestern Plaguelands\nEastern Plaguelands\nSilithus" },
		{ 60, "Up to 60: Winterspring\nEastern Plaguelands\nSilithus" },
		{ 62, "Up to 62: Hellfire Peninsula" },
		{ 64, "Up to 64: Zangarmarsh\nTerokkar Forest"},
		{ 65, "Up to 65: Terokkar Forest" },
		{ 66, "Up to 66: Terokkar Forest\nNagrand"},
		{ 67, "Up to 67: Nagrand"},
		{ 68, "Up to 68: Blade's Edge Mountains"},
		{ 70, "Up to 70: Blade's Edge Mountains\nNetherstorm\nShadowmoon Valley"},
		{ 72, "Up to 72: Howling Fjord\nBorean Tundra"},
		{ 74, "Up to 74: Grizzly Hills\nDragonblight"},
		{ 76, "Up to 76: Dragonblight\nZul'Drak"},
		{ 78, "Up to 78: Zul'Drak\nSholazar Basin"},
		{ 80, "Up to 80: The Storm Peaks\nIcecrown"},
	},
...
function tsugg_kiir(mit)
	DEFAULT_CHAT_FRAME:AddMessage(mit,0.1,1,1);
end
function tsugg_kiirwhite(mit)
	DEFAULT_CHAT_FRAME:AddMessage(mit,1,1,1);
end


function TSug_GetSuggestion(index, level)
	if Tsuggest[index] == nil then return "unknown" end
	
	for k, v in pairs( Tsuggest[index] ) do
...
--tsugg_kiir(TSug_GetSuggestion("Leveling", 33));
--tsugg_kiir(TSug_GetSuggestion("Riding", 275));


function tsug_leveling()
	tsugg_kiirwhite("Leveling:")
	tsugg_kiir(TSug_GetSuggestion("Leveling", UnitLevel("player")));
end
function tsug_prof()
o=0;
for i = 1, GetNumSkillLines() do
		...
		if isHeader then
			...
			tsugg_kiirwhite(skillName..": ");
			tsugg_kiir(TSug_GetSuggestion(skillName, skillRank));
			end
		end
end
end

function tsug_riding()
ridinglevel=0;
o=0;tsugg_kiirwhite("Riding: ");
for i = 1, GetNumSkillLines() do
...
function tsugg_command(cmd)
tsugg_kiirwhite("T-Suggestions "..tsugver);
if (cmd=="level") then
		tsug_leveling()
	end;
if (cmd=="prof") then
...
if (cmd=="all") then
		tsug_leveling();tsug_prof();
	end;
if (cmd=="") then 
 tsugg_kiir("/level -leveling tip");
 tsugg_kiir("/prof -all primary and secondary prof tip");
 tsugg_kiir("/riding -riding tip");
end;
end;
...
SLASH_TSUGG1 = "/ts";
SLASH_TSUGG2 = "/tsugg";
SLASH_TSUGG3 = "/tsuggest";
SlashCmdList["TSUGG"] = tsugg_command;
