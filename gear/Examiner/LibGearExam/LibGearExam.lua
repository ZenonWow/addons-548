...
-- Item Link Patterns -- Item strings currently have 11 parameters (5.1)
-- FORMAT  ->  item:itemId:enchantId:gemId1:gemId2:gemId3:gemId4:suffixId:uniqueId:linkLevel:reforgeId:upgradeId
LGE.ITEMLINK_PATTERN = "(item:[^|]+)";					-- Takes the raw itemLink from the full itemString
LGE.ITEMLINK_PATTERN_ID = "item:(%d+)";					-- Matches the itemLevel
LGE.ITEMLINK_PATTERN_ENCHANT = "item:%d+:(%d+)";		-- Matches the enchantId
LGE.ITEMLINK_PATTERN_LEVEL = "(%d+)(:%-?%d+:%-?%d+)$";	-- Matches the linkLevel -- This *must* be updated if another parameter gets added to item strings
--LGE.ITEMLINK_PATTERN_UPGRADE = ":(%d+)$";				-- Matches the upgradeId
--LGE.ITEMLINK_PATTERN_FULL = "item:(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)";	-- Az: TMP needs to expect "-" to work. Use "%-?" for that.
--LGE.ITEMLINK_PATTERN_FULL = "item:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^:]+:[^|]+";	-- Az: TMP alternative way of matching all
...
--AzMsg("set pattern match = "..setFound);
				for n = i + 1, self.Tip:NumLines() do
					...
					local setBonusIndex = tonumber(text:match(self.SetBonusTokenInactive));
...
function LGE:DoLineNeedScan(tipLine,scanSetBonuses)
	...
	text = text:gsub("|c%x%x%x%x%x%x%x%x",""):gsub(",","");
