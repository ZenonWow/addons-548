...
function CombatMode:OnEnable()
	...
	--self:RegisterEvent("ADDON_LOADED", function (event, addonName)  self:LogInit('CombatMode:ADDON_LOADED('.. self.colors.green .. addonName ..'|r)') ; self:HookUpFrames()  end)
	self:RegisterEvent("PLAYER_ENTERING_WORLD", function (event)  self:LogInit('CombatMode:'.. event ..'()') ; self:HookUpFrames()  end)
	--[[
...
function CombatMode:ProfileChanged()
	...
	-- smarttargeting, overrideBindings
...
CombatMode.overridden = { Mouselook = {}, CombatMode = CreateFrame("Frame"), AutoRun = CreateFrame("Frame") }
CombatMode.overridden.CombatMode:Hide()
CombatMode.overridden.AutoRun:Hide()
...
CombatMode.overrideBindings = {
	...
	All = {
		--MoveAndSteer = 'CameraOrSelectOrMove',
	},
}
--[[
/run  SetMouselookOverrideBinding('X', 'COMBATMODE_HOLD')
/run  SetMouselookOverrideBinding('X')
/run  CombatMode.overridden.CombatMode:Show()
--]]



function CombatMode.overridden.Mouselook:BindOverrides(fromCmd, toCmd)
	local fromUpper, toUpper= fromCmd:upper(), toCmd:upper()
	self.overrides= self.overrides  or  {}
	local oldKeys= self.overrides[fromCmd]  or  {}
	local newKeys= { GetBindingKey(fromUpper) }
	
	for  key,_  in  pairs(oldKeys)  do  if  not newKeys[key]  then
		-- Remove previous overrides
		SetMouselookOverrideBinding(key, nil)
	end end
	
	for  key,_  in  pairs(newKeys)  do  if  not oldKeys[key]  then
		-- New overrides
	...
	self.overrides[fromCmd]= newKeys
end	


function CombatMode.overridden.CombatMode:BindOverrides(fromCmd, toCmd)
	local frame= self
	local fromUpper, toUpper= fromCmd:upper(), toCmd:upper()
	frame.overrides= frame.overrides  or  {}
	local oldKeys= frame.overrides[fromCmd]  or  {}
	local newKeys= { GetBindingKey(fromUpper) }
	...
	for  key,_  in  pairs(newKeys)  do  if  not oldKeys[key]  then
		-- New overrides
		SetOverrideBinding(frame, false, key, toUpper)
	end end
	-- Make it nil if empty
	if  not next(newKeys)  then  newKeys= nil  end
	
	frame.overrides[fromCmd]= newKeys
end

CombatMode.overridden.AutoRun.BindOverrides = CombatMode.overridden.CombatMode.BindOverrides


function CombatMode:UpdateOverrideBindings()
	CombatMode.overridden.AutoRun:BindOverrides('MoveAndSteer', 'TurnOrAction')
if  true  then  return false  end

	--[[
/dump  CombatMode.overridden.Mouselook
/dump  CombatMode.overridden.AutoRun.overrides
	...
	--CombatMode.overridden.CombatMode:BindOverrides('MoveAndSteer', 'MoveForward')
	...
	--CombatMode.overridden.BindMouselookOverrides('InteractMouseover', 'CombatMode_Disable')
	...
	if  profile.bindings  then  for  key,cmd  in  pairs(profile.bindings)  do
		...
		elseif  origCmd == 'TURNORACTION'  or  origCmd == 'TARGETSCANENEMY'  then
			print('UpdateOverrideBindings():  '.. key ..' binding to '.. origCmd ..' not overridden with '.. cmd .. ' as it would cause stucking in Combat Mode.')
		else
