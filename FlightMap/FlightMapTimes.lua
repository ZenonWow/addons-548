...
function FlightMapTimes_SelectGossipOption(index, ...)
    ...
    if optionTitle and optionType == 'gossip' or optionType == 'taxi' then
        ...
        if map[unitName] then
            local duration,exact = FlightMapUtil.getFlightTime(unitName, optionTitle, map) --map[unitName].Flights[optionTitle]; --#
            FlightMapTimesRecorderFrame.duration = duration;
	    FlightMapTimesRecorderFrame.exact = exact
        end
...
function FlightMapTimes_TakeTaxiNode(id)
    ...
    -- Get duration
    FlightMapTimesRecorderFrame.duration, FlightMapTimesRecorderFrame.exact = FlightMapUtil.getFlightTime(source, dest)
    ...
    if FlightMap.Opts.confirmFlights then
        ...
        if map[source] and map[source].Flights[dest] then 
            local seconds = FlightMapUtil.getFlightTime(source, dest)
...
local function lSaveFlightTime(length, from, to, flightType)
	...
	if isInstance then
		print(string.format("Not recorded duration of %.1f; you are in %s", length, instanceType))
		return
	end
	...
	if map[from] then
		FlightMapUtil.setFlightTime(from, to, length, map)	
	end
end
...
function FlightMapTimes_OnUpdate(self)
	...
	local timing
		...
		sparkPos = ( (now - self.startTime) / (self.endTime - self.startTime) )
		               * (FlightMapTimesFrame:GetWidth() or 1); -- default is 195, generified by Moncai; DIV BY ZERO!!!
	    end
...
function FlightMapTimes_EndFlight(self)
