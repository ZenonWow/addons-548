...
local addonpanels = {}
local addonpanels_select = {none=nil}
do
	local name, title, notes, enabled, loadable, reason, security, state
	for i, n ipairs({"OptionHouse","ACP","stAddonManager"}) do
		addonpanels[n] = nil -- nil = not present, false = present but not loaded yet, true = present and loaded
		name, title, notes, enabled, loadable, reason, security = GetAddOnInfo(n)
		if ( loadable ) then
			addonpanels[n] = ( IsAddOnLoaded(n)==true )
		end
		state = (addonpanels[n]==nil and " (not found)") or ""
		addonpanels_select[n] = n.. state
	}
end
