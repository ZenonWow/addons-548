...
function Primary:OnMouseUp ( Button )
	...
	if ( IsModifiedClick( "DEV_FRAMES_OUTLINE" ) ) then
		-- Outline frame
		_Dev.Outline.Toggle( Target, L.FRAMES_MOUSEFOCUS );
	end
	if ( IsModifiedClick( "DEV_FRAMES_NAME" ) ) then
	...
	if ( IsModifiedClick( "DEV_FRAMES_DUMP" ) ) then
		-- Dump frame in full
...
SlashCmdList[ "DEV_FRAMESTOGGLE" ] = NS.ToggleSlashCommand;