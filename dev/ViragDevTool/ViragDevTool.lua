...
ViragDevTool = {
    ...
    CMD = {
        ...
        HELP = function()
            ...
            local cFix = function(str)
                ...
                result = string.gsub(result, "center", a3("center"))
            ...
            help[cFix("10 /vdt center")] = cFix("Reset main frame position if you lost it for some reason")
            help[cFix("11 /vdt vdt_reset_wnd")] = cFix("Reset main frame position if you lost it for some reason")
            local sortedTable = {}
    ...
    -- Default settings
    -- this variable will be used as fallback/default for settings not changed by user
    -- the actual settings are in the saved var ViragDevTool_Settings
    default_settings = {
...
function ViragDevToolLinkedList:RemoveNode(node)
    self:RemoveChildNodes(node)
    
		local prevNode = nil
    if  self.first == node  then
				-- deleted first node -> no prevNode
        self.first = node.next
		else
        prevNode = self.first
				-- find previous node by iterating linked list
        while  prevNode  and  prevNode.next ~= node
				do  prevNode = prevNode.next  end
				
        if  prevNode ~= nil  then
				    -- found it -> link to removed node's next -> skip removed node
            prevNode.next = node.next
				else
				    -- it's an inconsistency error
						ViragDevTool:print('Error - Removed item missing from list: "' .. tostring(node.name) .. '"')
        end
		end
		
		if  self.last == node  then
		    -- deleted the last node -> prevNode is the new last node
				--assert(prevNode == nil  or  prevNode.next == nil)  -- last node has no next node
		    self.last = prevNode
    end
...
function ViragDevTool:RemoveCell(info)
    self.list:RemoveNode(info)
    self:UpdateMainTableUI()
end
...
function ViragDevTool:ToggleUI()
    ...
    self.settings.isWndOpen = self.wndRef:IsVisible()  and  true  or  false
end
...
function ViragDevTool:ToggleSidebar()
    ...
    self.settings.isSideBarOpen = self.wndRef.sideFrame:IsVisible()  and  true  or  false
    self:UpdateSideBarUI()
end
...
function ViragDevTool:SubmitEditBox(msg)
...
function ViragDevTool:SetMainTableButtonScript(button, info)
    ...
    local leftClickFn = function()
			ChatEdit_InsertLink( tostring(info.value) )
    end
    ...
    --]]
    button:SetScript("OnMouseUp", function(this, mouseButton, down)  ViragDevTool.MainTableButtonClick(this, mouseButton, down, info)  end)  -- OnClick
    --button:RegisterForClicks('AnyUp')
end

-- intentional static method to avoid confusing self and this
-- use  'this'  instead of  'self'  to reference the clicked Button object
function  ViragDevTool.MainTableButtonClick(this, mouseButton, down, info)
	local pre, out = '', ''
	if  mouseButton == "RightButton"  then
		if  IsControlKeyDown()  then
			ViragDevTool:RemoveCell(info)
		else
			local nameButton = this:GetParent().nameButton
			local valueButton = this:GetParent().valueButton
			pre= nameButton:GetText() .. " = "
			out= valueButton:GetText()
		end
	else
		if  IsShiftKeyDown()  then
			pre= 'Var Name:  '
			out= tostring(info.name)
		elseif  IsControlKeyDown()  then
			if  type(info.value) == 'function'  then  
				pre= 'Calling  ' .. info.name .. '()'
				ViragDevTool:TryCallFunction(info)
			end
		elseif  not IsModifierKeyDown()  then
			if  type(info.value) == "table"  then
				if  info.expanded
				then  ViragDevTool:ColapseCell(info)
				else  ViragDevTool:ExpandCell(info)
				end
			else
				pre= 'Var  ' .. tostring(info.name) .. '= '
				out= tostring(info.value)
			end
		end
	end

	ViragDevTool:print( pre .. out )
	if  out  then  ChatEdit_InsertLink( out )  end
end
...
function ViragDevTool:OnLoad(mainFrame)
    ...
    self.wndRef:SetScript("OnEvent", function(this, event, addonName, ...)
        ...
        if event == "PLAYER_ENTERING_WORLD" then
						ViragDevTool.wndRef:UnregisterEvent("PLAYER_ENTERING_WORLD")
            self:OnAddonSettingsLoaded()
        end
    end);
...
local function  SetFallbackToDefaults(settings, defaults)		-- settings, defaults
	local  s= settings
	for  k,dV  in  pairs(defaults)  do
		if  s[k]  and  type(s[k]) ~= type(dV)  then
			-- validating current setting and reseting if type changed
		  -- save outdated setting for reference for the user
			s['_OUTDATED_' .. k]= s[k]
			-- reset outdated setting
			s[k]= nil
		end
		--[[
		if  type(dV) == 'table'  then
			-- recusively link child tables
			if  not s[k]  then  s[k]= {}  end
			SetFallbackToDefaults(s[k], dV)
		end
		--]]
	end
	-- set fallback to default settings
	if  settings ~= defaults
	then  setmetatable( settings, { __index= defaults } )  end
end

function ViragDevTool:InitSetting(key)
	-- if  self.settings[key] == self.default_settings[key]  then
	if  not rawget(self.settings, key)  then
		-- clone self.default_settings[key]
		self.settings[key]= {unpack(self.settings[key])}
	end
end


function ViragDevTool:OnAddonSettingsLoaded()
    if  not ViragDevTool_Settings
		then  ViragDevTool_Settings= {}  end
    local s= ViragDevTool_Settings
		...
		-- set fallback to default settings
		s.colors= s.colors or {}
		SetFallbackToDefaults(s, self.default_settings)
		SetFallbackToDefaults(s.colors, self.default_settings.colors)
...
function ViragDevTool:print(strText)
		if  not strText  or  strText == ''  then  return  end
    print(self.colors.darkred .. "[Virag's DT]: " .. self.colors.white .. strText .. self.colors.restore)
end
