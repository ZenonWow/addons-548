		...
		config.options.args.scanning.plugins.cache = {
			cache = {
				...
				args = {
					...
					location = config.toggle("Record location on cache hit", "Record the mob's location when the cache triggers for it. If this isn't set, it'll wait until you target it and are within interaction range to store the location. Note: if this is disabled, Mysterious Camel Figurine locations will not be saved!", 30),
				},
			},
		}
	end
...
function module:NotifyIfNeeded(id, zone)
	...
	local newloc = false
	if not globaldb.mob_tameable[id] then--Special exception for blood seeker
