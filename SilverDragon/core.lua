...
function addon:CHAT_MSG_SYSTEM(event, msg)
	if (not self.db.profile.serverreboot) then return end
	if not InCombatLockdown() and msg == IDLE_MESSAGE then
	...
	realmTimer = GetTime()
	if msg:find("realm") and (msg:find("restart") and not msg:find("Cross") or msg:find("maintenance")) then--Message contains both "realm" and "restart" or "realm" and "maintenance" in same sentance it's probalby right message
...
function addon:NotifyMob(id, name, zone, x, y, is_dead, is_new_location, source, unit, silent, guid)
--	local mobguid = guid or 0
	if silent then
	...
	if lastseen[id..zone] and time() < lastseen[id..zone] + self.db.profile.delay then
