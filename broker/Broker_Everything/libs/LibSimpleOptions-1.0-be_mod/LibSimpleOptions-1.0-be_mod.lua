...
Modified by: Hizuro (hizuro@gmx.net)
]]
...
local function generic_OnEnter(self)
	...
	if type(self.tooltipText)=="table" then
		...
		for i,v in ipairs(self.tooltipText) do
			if type(v)=="string" and strlen(v)>1 then
				GameTooltip:AddLine(v,r,g,b,1)
			end
			r,g,b = 1,1,1
		end
		GameTooltip:Show()
...
do
	...
	function panelMeta:MakeToggle(...)
		...
		if type(args.name) ~= "string" then
			error(("name must be %q, got %q (%s)"):format("string", type(args.name), tostring(args.name)), 2)
		elseif type(args.description) ~= "string" and type(args.description) ~= "table" then
			error(("description must be %s, got %q (%s)"):format("string or table", type(args.description), tostring(args.description)), 2)
