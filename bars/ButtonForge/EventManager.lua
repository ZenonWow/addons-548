...
function Glow:OnEvent(Event, Arg1)
	...
	if ((Arg1 == 88684 or Arg1 == 88685) and select(5, GetTalentInfo(2, 16)) == 1) then	-- Holy Word: Serenity or Holy Word: Sanctuary and Talent Revelations
		Name = GetSpellInfo(88625);	--Holy Word: Chastise (since this is what will actually allow us to match)
		-- Of note, Santuary glow will also trigger chastise glow, but this is not the case for serenity (so rather than relying on it both will be converted to chastise)
	end
...
function Delay:OnUpdate(Elapsed)
	...
	if (self.DelayTime > ButtonForgeGlobalSettings["MacroCheckDelay"]) then
