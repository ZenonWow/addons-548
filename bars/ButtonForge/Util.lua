...
function Util.ApplySlashCommands(Commands, Bar)
	...
	if (Commands["-scale"]) then
		Bar:SetScale(Commands["-scale"][1]);
	end
	
	if (Commands["-rows"]) then
		Bar:SetNumButtons(Bar.BarSave["Cols"], Commands["-rows"][1]);
	end
	
	if (Commands["-cols"]) then
		Bar:SetNumButtons(Commands["-cols"][1], Bar.BarSave["Rows"]);
	end
	
	if (Commands["-coords"]) then
		Bar:SetPosition(Commands["-coords"][1], Commands["-coords"][2]);
	end
	...
	if (Commands["-gap"]) then
		Bar:SetButtonGap(Commands["-gap"][1]);
	end
	...
	if (Commands["-macrocheckdelay"]) then
		ButtonForgeGlobalSettings["MacroCheckDelay"] = Commands["-macrocheckdelay"][1];
	end
