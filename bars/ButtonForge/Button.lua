...
function Button:SetEnvSpell(Id, NameRank, Name, Book)
	...
	if (Id == 88625) then	--Holy Word: Chastise
		-- special code for handling this particular spell...
		self.UpdateTexture = Button.UpdateTextureHolyWordChastise;
		self.UpdateTooltipFunc = Button.UpdateTooltipHolyWordChastise;
	end
	...
	self.SpellName 		= Name;
	self.SpellBook 		= Book;
	self.Texture 		= GetSpellBookItemTexture(NameRank);	--safe provided NameRank ~= nil
...
function Button:SetAttributes(Type, Value)
	...
	elseif (Type == "battlepet") then
		self.Widget:SetAttribute("type", "macro");
		self.Widget:SetAttribute("macrotext", "/run C_PetJournal.SummonPetByGUID(\""..Value.."\")");
	end
end
...
function Button:UpdateTextureHolyWordChastise()

	if (select(5, GetTalentInfo(2, 16)) == 1) then
		local SpellName = GetSpellInfo(81208);	--To get a locale safe name of Chakra: Serenity

		if (UnitBuff("player", SpellName)) then
			self.WIcon:SetTexture("Interface/Icons/Spell_holy_persuitofjustice");
			return;
		end
		
		SpellName = GetSpellInfo(81206);	--Chakra: Sanctuary
		if (UnitBuff("player", SpellName)) then
			self.WIcon:SetTexture("Interface/Icons/Spell_holy_divineprovidence");
			return;
		end
	end
	self.WIcon:SetTexture(self.Texture);
...
function Button:UpdateCooldownSpell()
	...
	if (Start ~= nil) then
		...
		CooldownFrame_SetTimer(self.WCooldown, Start, Duration, Enable, Charges, MaxCharges);
	else
		CooldownFrame_SetTimer(self.WCooldown, 0, 0, 0);
		self.WCooldown:Hide();
	end
...
function Button:UpdateCooldownBonusAction()
	if (HasOverrideActionBar() or HasVehicleActionBar()) then
		local action = self.Widget:GetAttribute("action");
		CooldownFrame_SetTimer(self.WCooldown, GetActionCooldown(action));
	else
		self.WCooldown:Hide();
	end
end
...
function Button:UpdateTooltipHolyWordChastise()
	self = self.ParentButton or self;	--This is a sneaky cheat incase the widget was used to get here...
	...
	if (select(5, GetTalentInfo(2, 16)) == 1) then
		...
		if (UnitBuff("player", SpellName)) then
			GameTooltip:SetSpellByID(88684);	--Holy Word: Serenity
			return;
		end
		
		SpellName = GetSpellInfo(81206);	--Chakra: Sanctuary
		if (UnitBuff("player", SpellName)) then
			GameTooltip:SetSpellByID(88685);	--Holy Word: Sanctuary
			return;
		end
	end
...
function Button:RefreshMacro()
	...
	if (self.Mode == "macro") then	
		...
		if (Util.IncBetween(self.MacroIndex - 1, 1, AccMacros) or Util.IncBetween(self.MacroIndex - 1, Const.MAX_ACCOUNT_MACROS + 1, Const.MAX_ACCOUNT_MACROS + CharMacros)) then
...
function Button:RefreshSpell()
	...
	if (self.Mode == "spell") then
		self.Texture = GetSpellBookItemTexture(self.SpellNameRank);
		self.Texture = self.Texture or "Interface/Icons/INV_Misc_QuestionMark";
		self:DisplayActive();
	end
end
