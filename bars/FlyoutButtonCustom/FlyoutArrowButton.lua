...
function FlyoutArrowButton_Attach(frame)
	...
	if (temp) and (type(temp) == "table") then
		for i, v in ipairs(temp) do
			...
			--print("currentSet "..tostring(currentSet)..": "..tostring(v.command)..", "..tostring(v.value)..", "..tostring(v.subValue)..", "..tostring(v.id))
			if ACCEPTABLE_COMMANDS[v.command] then
				...
				elseif (v.command == "battlepet") then
					if not(v.value) or type(v.value) ~= "string" or not(v.value:match("^0x")) then
						print(tostring(v.command).." "..tostring(v.subValue).." removed")
						btn:SaveCommand(nil, nil, nil, nil)
						btn:Set(nil, nil, nil, nil)
						btn:SetKey(nil)
...
function FlyoutArrowButton:PostClick(button)
	if not(InCombatLockdown()) then
		...
		elseif button == "RightButton" and not(FbcArrowBtnDirLocked) then
			...
			FlyoutButtonES:Set({"expandDir", GetActiveSpecGroup(), self:GetParent():GetName()}, expandDir)
		...
		for i = 1, #bl do
			if (bl[i].command) then
				if not(found) then
					...
					-- print "moving"
					print("FlyoutButtonCustom moving arrow button, drop it onto another arrow button")
