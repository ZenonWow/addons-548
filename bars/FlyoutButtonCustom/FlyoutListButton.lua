...
local preset_by_companion = {
	...
	["GetTexture"] 		= "GetTextureCompanion",
...
function FlyoutListButton:SetKey(key) -- binds the given key to the given button
	...
	FlyoutButtonES:Set({self.actnBtnName, GetActiveSpecGroup(), self.index, "keybind"}, key)
end
...
function FlyoutListButton:UpdateTooltipCompanion()
	if (self.id) then
		GameTooltip:SetSpellByID(self.id)
...
function FlyoutListButton:SetTooltipCompanion()
...
function FlyoutListButton:GetTextureCompanion()
	local _, _, _, texture = FlyoutButton_GetCompanionInfoByName(self.value, self.subValue)
	return texture
end
...
function FlyoutListButton:UpdateCheckedCompanion()
	local value, subValue = self.value, self.subValue
	local _, _, _, _, result = FlyoutButton_GetCompanionInfoByName(value, subValue)
...
function FlyoutListButton:UpdateCooldownSpell()
	--print(tostring(self.command)..", "..tostring(self.value)..", "..tostring(self.subValue)..", "..tostring(self.id))
	local start, duration, enable = GetSpellCooldown(self.value) --GetSpellCooldown(id)
	if start then
...
function FlyoutListButton:GetUsableSpell()
	return IsUsableSpell(self.id) --self.value
end
...
function FlyoutListButton:GetUsableComapnion()
...
function FlyoutListButton:InRangeCompanion()
	return nil
end
...
function FlyoutListButton:OnDragStart()
	...
	FlyoutButtonES:Set({self.actnBtnName, GetActiveSpecGroup(), self.index}, {})
