...
function FlyoutButtonCustom_ButtonFacadeCallback(self, SkinID, Gloss, Backdrop, Group, Button, Colors)
	...
	if not(Group) then
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, "SkinID"}, SkinID)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, "Gloss"}, Gloss)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, "Backdrop"}, Backdrop)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, "Colors"}, Colors)
	else
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, Group, "SkinID"}, SkinID)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, Group, "Gloss"}, Gloss)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, Group, "Backdrop"}, Backdrop)
		FlyoutButtonES:Set({BF_SETTINGS_GROUP, Group, "Colors"}, Colors)
	end
end
...
local function EventSpellsChanged()
	FlyoutButton_CacheSpells()
end
...
local OnShowHookFrames = {SpellBookFrame, PaperDollFrame} --, MacroFrame, PetJournalParent
...
local function SettingsHookFunc()
	...
	hooksecurefunc("TogglePetJournal", function()
		if PetJournalParent:IsShown() then
			FlyoutButtonCustom_SettingsIncCounter()
			PetJournalParent:HookScript("OnHide", FlyoutButtonCustom_SettingsDecCounter)
		end
	end)
end
...
local function Init()
	...
	if (LBF) then
		FbcLBFMasterGroup = LBF:Group("FlyoutButtonCustom")
		LBF:RegisterSkinCallback("FlyoutButtonCustom", FlyoutButtonCustom_ButtonFacadeCallback, BF_Table)
		FbcLBFMasterGroup:Skin(
			FlyoutButtonES:Get({BF_SETTINGS_GROUP, "SkinID"}, nil),
			FlyoutButtonES:Get({BF_SETTINGS_GROUP, "Gloss"}, nil),
			FlyoutButtonES:Get({BF_SETTINGS_GROUP, "Backdrop"}, nil),
			FlyoutButtonES:Get({BF_SETTINGS_GROUP, "Colors"}, nil))
	end
