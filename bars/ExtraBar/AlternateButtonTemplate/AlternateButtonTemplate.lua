...
local CompanionInCache = false;		--this flag is a quick hack to make companions work better!
...
ABTFrame:RegisterEvent("PET_JOURNAL_LIST_UPDATE");

local MacroDelayCompleted = false;
...
function ABTMethods_PostClick(self)
	...
	if (self.cursorCommand) then
		...
		local set = GetActiveSpecGroup();
		if (self["set"..set.."type"] == "battlepet") then
...
function ABTMethods_OnReceiveDrag(self)
	...
	if ((not InCombatLockdown()) and (command == "spell" or command == "item" or command == "macro" or command == "companion" or command == "battlepet")) then
...
local ATBFrame_MacroCheckDelay = CreateFrame("FRAME");
function ABTMethods_LoadAll(entries, settings)
	...
	ATBFrame_MacroCheckDelay:SetScript("OnUpdate", ATBFrame_MacroCheckDelay.OnUpdate);

end



ATBFrame_MacroCheckDelay.DelayTime = 0;
function ATBFrame_MacroCheckDelay:OnUpdate(Elapsed)
	self.DelayTime = self.DelayTime + Elapsed;
	if (self.DelayTime > 5) then
		self:SetScript("OnUpdate", nil);
		MacroDelayCompleted = true;
		ABTFrame:OnEvent("UPDATE_MACROS");
	end
end


function ABTMethods_SetButtonFacadeGroup(self, group)
...
function ABTMethods_UpdateTexture(self)
	...
	elseif (command == "battlepet") then
		texture	= select(9, C_PetJournal.GetPetInfoByPetID(value));
	end
...
function ABTMethods_UpdateChecked(self)
	...
	elseif (command == "battlepet") then
		local Active = value == C_PetJournal.GetSummonedPetGUID();
...
function ABTMethods_UpdateCooldown(self)
	...
	if (command == "spell" and value) then
		start, duration, enable = GetSpellCooldown(value);
	elseif (command == "item") then
		local id = ABTMethods_GetItemInfoFromLink(select(2, GetItemInfo(value)));
		if (id) then
...
function ABTMethods_SetCommand(self, command, value, actualType, name, id, noSave, set)
	...
	--[[This section of code was to avoid unknown spells sticking around, as it turns out it was causing
		some spells to be dropped when changing spec; it appears to be safe to leave an unknown spell on the bar, so I will leave this code commented (and perhaps clear later)
	]]--
	--if (command == "spell" and actualType ~= "MOUNT" and actualType ~= "CRITTER") then
	...
	local currentSet = GetActiveSpecGroup();
	if (not set) then
		set = currentSet;
	end
	...
	if (set == currentSet) then
		...
		if (command == "battlepet") then
			self:SetAttribute("macrotext", "/run C_PetJournal.SummonPetByGUID(\""..value.."\")");
			self:SetAttribute("type", "macro");
		else
...
function ABTMethods_SetFromCursorInfo(self, command, value, subValue)
	...
	elseif (command == "battlepet") then
		ABTMethods_SetCommand(self, command, value, command, "", "");
	end
...
function ABTMethods_SetCursor(command, value)
	...
	if (command == "spell") then
		local spellId = ABTMethods_FindSpellID(value);
		if (spellId) then
			PickupSpellBookItem(spellId, BOOKTYPE_SPELL);
		end
	elseif (command == "item") then
		PickupItem(value);
	elseif (command == "macro") then
...
function ABTMethods_GetSpellName(spell, ...)
	local name, rank = GetSpellBookItemName(spell, ...);
...
function ABTMethods_FindCompanionID(compType, compName)
	for i = 1, GetNumCompanions(compType) do
		...
		if (name == compName) then
			CompanionInCache = true;
			return i;
		elseif (not name) then
			CompanionInCache = false;	--If we hit here, not all companion info is available yet, this flag will stop the system making any un-intended changes
			return nil;
		end
	end
	CompanionInCache = true; --actually we shouldn't really get here, but if we do we at least have all available companions in the cache
	return nil;
end
...
function ABTMethods_UpdateSpell(self)
	...
	--I've turned this code off as it would appear that rank promotion is almost redundant
...
function ABTMethods_UpdateMacro(self)
	if (InCombatLockdown()
		or not ABTFrame.worldLoaded
		or not MacroDelayCompleted) then
		return;
	end
...
function ABTMethods_UpdateCompanion(self)
	...
	if (self.set1actualtype == "MOUNT" or self.set1actualtype == "CRITTER") then
		...
		if (CompanionInCache) then
			if (id == nil) then
				--we genuinely didn't find the companion, so remove from the button
				ABTMethods_SetCommand(self, "none", "", "", "", "", false, 1);
			elseif (id ~= self.set1id) then
				ABTMethods_SetCommand(self, "spell", self.set1value, self.set1actualtype, self.set1name, id, false, 1);
			end
		elseif (self.set1id == nil) then
			self.set1id = 1;	--quick a dirty work around in case the id has previously been trampled
		end
	end
	if (self.set2actualtype == "MOUNT" or self.set2actualtype == "CRITTER") then
		...
		if (CompanionInCache) then
			if (id == nil) then
				ABTMethods_SetCommand(self, "none", "", "", "", "", false, 2);
			elseif (id ~= self.set2id) then
				ABTMethods_SetCommand(self, "spell", self.set2value, self.set2actualtype, self.set2name, id, false, 2);
			end
		elseif (self.set2id == nil) then
			self.set2id = 1;	--quick a dirty work around in case the id has previously been trampled
		end
	end
end
