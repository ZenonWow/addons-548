...
function EBCore:OnEvent(event, arg1)
	...
	if (EBCore.WorldEntered and EBCore.VariablesLoaded) then
		if (LibStub) then
			EBCore.LBF = LibStub("LibButtonFacade", true);
			if (EBCore.LBF) then
				EBCore.LBFMasterGroup = EBCore.LBF:Group("Extra Bars");
				EBCore.LBFBar1Group = EBCore.LBF:Group("Extra Bars", "Bar 1");
				EBCore.LBFBar2Group = EBCore.LBF:Group("Extra Bars", "Bar 2");
				EBCore.LBFBar3Group = EBCore.LBF:Group("Extra Bars", "Bar 3");
				EBCore.LBFBar4Group = EBCore.LBF:Group("Extra Bars", "Bar 4");
				EBCore.LBF:RegisterSkinCallback("Extra Bars", EBCore.ButtonFacadeCallback, EBCore);
			end
		end
		EBCore:UnregisterEvent("PLAYER_ENTERING_WORLD");
		EBCore:UnregisterEvent("VARIABLES_LOADED");
		EBCore:SetScript("OnEvent", nil);
		self:Init();
	end
end
EBCore:SetScript("OnEvent", EBCore.OnEvent);

function EBCore:ButtonFacadeCallback(SkinID, Gloss, Backdrop, Group, Button, Colors)
	...
	if not Group then
		ExtraBar_Config["SkinID"] = SkinID;
		ExtraBar_Config["Gloss"] = Gloss;
		ExtraBar_Config["Backdrop"] = Backdrop;
		ExtraBar_Config["Colors"] = Colors;
	else
		if (not ExtraBar_Config[Group.."LBF"]) then
			ExtraBar_Config[Group.."LBF"] = {};
		end
		ExtraBar_Config[Group.."LBF"]["SkinID"] = SkinID;
		ExtraBar_Config[Group.."LBF"]["Gloss"] = Gloss;
		ExtraBar_Config[Group.."LBF"]["Backdrop"] = Backdrop;
		ExtraBar_Config[Group.."LBF"]["Colors"] = Colors;
	end
end

function EBCore:Init()
	self.UpdateSavedData();
	if (EBCore.LBFMasterGroup and ExtraBar_Config["SkinID"]) then
		EBCore.LBFMasterGroup:SetSkin(ExtraBar_Config["SkinID"], ExtraBar_Config["Gloss"], ExtraBar_Config["Backdrop"], ExtraBar_Config["Colors"]);	--we do not use Skin, since that would blow away the saved data for each group, this is actually purely cosmetic for the Facade interface
	end
...
--Init the bar itself
	...
	if (EBCore["LBFBar"..num.."Group"]) then
		local buttons = {bar:GetChildren()};
		for i,v in ipairs(buttons) do
			if (v:GetObjectType() == "CheckButton") then
				ABTMethods_SetButtonFacadeGroup(v, EBCore["LBFBar"..num.."Group"]);
			end
		end
		local settings = ExtraBar_Config["Bar "..num.."LBF"];
		if (settings) then
			EBCore["LBFBar"..num.."Group"]:Skin(settings.SkinID, settings.Gloss, settings.Backdrop, settings.Colors);
		end
	end
...
function EBCore.UpdateVisibilityStateDriver(num)
	...
	elseif (ExtraBar_Config["EB"..num.."CfgHideInVehicle"]) then
		UnregisterStateDriver(bar, "visibility");
		RegisterStateDriver(bar, "visibility", "[petbattle][target=vehicle, exists][overridebar] hide; show" );
	else
		UnregisterStateDriver(bar, "visibility");
